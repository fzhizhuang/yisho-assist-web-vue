...
<template>
  ...
  <div class="chat-message-item" v-for="message in messages" :key="message.messageId">
    ...
    <div class="chat-message-item-content">
      <MarkdownViewer :markdownContent="message?.content" v-if="message.type === 'text'" />
      <a-image :src="message?.content" width="256px" height="256px" v-else />
    </div>
    <div class="chat-message-item-foot">
      <!-- 复制 -->
      <a-tooltip content="复制" position="bottom">
        <a-button type="dashed" shape="circle" @click="handleCopy(message?.content)">
          <IconCopy />
        </a-button>
      </a-tooltip>
      <!-- 删除 -->
      <a-tooltip content="删除" position="bottom">
        <a-button type="dashed" shape="circle" @click="handleDeleteMessage(message?.messageId)">
          <IconDelete />
        </a-button>
      </a-tooltip>
      <!-- 日期 -->
